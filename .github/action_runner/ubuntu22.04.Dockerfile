...
RUN mv $RUN_SCRIPT_PATH /home/$USER_NAME/actions-runner/_run.sh && \
    ...
    echo './config.sh --url https://github.com/nhandyal/sapling --token "$token" --unattended --labels self-hosted,ubuntu-22.04,arm64 --no-default-labels --replace'  >> $RUN_SCRIPT_PATH && \
